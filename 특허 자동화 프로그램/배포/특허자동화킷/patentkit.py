...
문체규칙 = [
    ...
    # 가이드 1.3 개방형 표현. 확정적 기재는 권리범위를 좁힌다
    ("확정적 표현", re.compile(
        r"반드시|오직|만으로\s*(?:이루어|구성)|(?<![가-힣])구성된다|한정된다(?!\s*고)"
        r"|(?:하여야|해야)\s*한다"), "warn"),
]

# 청구항의 구성요소로 셀 말. 「~부·모듈·유닛·엔진」 으로 끝나는 낱말을 센다
꼬리 = ("으로써", "으로서", "에서는", "에게서", "으로", "에서", "에게", "까지", "부터",
        "라도", "이나", "와", "과", "을", "를", "이", "가", "은", "는", "의", "에",
        "로", "도", "만", "나")
# 「일부·내부」 처럼 부로 끝나기만 하는 보통말은 구성요소가 아니다
부아닌말 = {"일부", "전부", "내부", "외부", "상부", "하부", "세부", "대부", "저부",
            "전면부", "후면부", "일부분", "대부분", "각부", "해당부"}
낱말 = re.compile(r"[가-힣]{3,24}")
하드웨어어 = ("프로세서", "메모리", "저장부", "연산", "통신부")


def 구성요소뽑기(본문: str) -> set[str]:
    """청구항에서 「~부」 꼴 구성요소를 뽑는다. 뒤에 붙은 조사는 떼어 낸다."""
    모음 = set()
    for 말 in 낱말.findall(본문):
        for j in 꼬리:
            if 말.endswith(j) and len(말) - len(j) >= 3:
                말 = 말[:-len(j)]
                break
        if 말.endswith(("부", "모듈", "유닛", "엔진")) and 말 not in 부아닌말:
            모음.add(말)
    return 모음
...
def 가이드검사(spec: str, claims: str, 절: dict, rep: 보고):
    """제작 참고자료의 「특허명세서작성 가이드라인」 이 따로 요구하는 것을 본다.

    ○ 6.1 뒷받침 요건 — 청구항 구성요소가 발명의 설명에 없으면 거절이유다
    ○ 7.1 요약서 분량 — 가이드 권장은 200자 내외다
    ○ 5.1 하드웨어 결합 — AI·SW 발명은 하드웨어와 묶어 적는다
    """
    본문 = claims.split("## 회피 논거")[0] if claims else ""
    부품 = 구성요소뽑기(본문)
    없는것 = sorted(t for t in 부품 if t not in spec)
    if not 부품:
        rep.통과("뒷받침 요건", "셀 구성요소 없음")
    else:
        rep.판정("뒷받침 요건",
                 [f"청구항의 「{t}」 가 발명의 설명에 없다" for t in 없는것],
                 f"구성요소 {len(부품)}개 모두 설명에 있다")

    # 【요약서】 는 머리표만이고 알맹이는 【요약】 에 있다. 긴 쪽을 본다
    요약 = max(절.get("요약", ""), 절.get("요약서", ""), key=len)
    글자 = len(re.sub(r"\s|\[\d{4}\]|【.*?】", "", 요약))
    if not 요약:
        rep.오류("요약서 분량", "요약서 절을 찾지 못했다")
    elif 글자 > 400:
        rep.경고("요약서 분량", f"{글자}자 · 가이드 권장은 200자 내외다")
    else:
        rep.통과("요약서 분량", f"{글자}자")

    있는어 = [w for w in 하드웨어어 if w in spec]
    if 있는어:
        rep.통과("하드웨어 결합", " · ".join(있는어))
    else:
        rep.경고("하드웨어 결합",
                 "프로세서·메모리 같은 하드웨어 기재가 없다 (가이드 5.1)")
